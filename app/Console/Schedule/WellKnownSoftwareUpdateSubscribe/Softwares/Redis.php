...
class Redis implements SoftwareInterface
{
    ...
    public function getVersion(): ?string
    {
        $data = $this->getJson();
        if ($data === 304) {
            return Common::getLastVersion(Software::Redis) ?: null;
        }
        if (!is_array($data)) {
            return null;
        }
        ...
        foreach ($data as $branch) {
            ...
            if (preg_match('/^(\d+\.\d+\.\d+)$/D', $branch['name'], $matches) === 1) {
    ...
    private function getJson(): array|int|null
    {
        ...
        Common::cacheLastModified(Software::Redis, $get->header('last-modified'));
        return $data;
    }
}
