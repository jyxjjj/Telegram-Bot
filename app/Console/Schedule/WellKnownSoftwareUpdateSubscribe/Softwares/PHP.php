...
class PHP implements SoftwareInterface
{
    ...
    public function getVersion(): ?string
    {
        $data = $this->getJson();
        if ($data === 304) {
            return Common::getLastVersion(Software::PHP) ?: null;
        }
        if (!is_array($data)) {
            return null;
        }
        ...
        foreach ($data as $branch) {
            ...
            if (preg_match('/^PHP-(\d+\.\d+\.\d+)$/iD', $branch['name'], $matches) === 1) {
    ...
    private function getJson(): array|int|null
    {
        ...
        Common::cacheLastModified(Software::PHP, $get->header('last-modified'));
        return $data;
    }
}
