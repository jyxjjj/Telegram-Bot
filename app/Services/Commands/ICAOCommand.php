...
class ICAOCommand extends BaseCommand
{
    public string $name = 'icao';
    public string $description = 'Search icao info.';
    public string $usage = '/icao {Search String}';
    ...
    private function find(array &$data, string $name): void
    {
        ...
        foreach ($json as $id => $airport) {
            if (str_contains(strtolower($airport['icao'] ?? ''), strtolower($name))) {
                $data['text'] = <<<EOF
Name: {$airport['name']}
IATA: {$airport['iata']}
ICAO: {$airport['icao']}
